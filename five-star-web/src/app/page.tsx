      ...
      <div style={{ background: CREAM_ALT }}>
        ...
        <section className={styles.hero} style={{ background: GRADIENT_HERO, position: "relative", overflow: "hidden", textAlign: "center" }}>
          <div className={styles.heroInner} style={{ position: "relative", zIndex: 1, margin: "0 auto" }}>
            ...
            <p style={{ fontSize: 14, lineHeight: 1.55, color: TEXT_MUTED, maxWidth: 520, margin: "0 auto 24px" }}>
        ...
        <section className={styles.sectionPad}>
          ...
          <div className={styles.howGrid} style={{ display: "grid" }}>
            {HOW_IT_WORKS.map((s) => (
              <div key={s.n} style={{ background: CREAM, borderRadius: RADIUS.lg, padding: 20 }}>
                <span
                  style={{
                    display: "inline-flex",
                    width: 34,
                ...
                <h3 style={{ fontSize: 14, fontWeight: 700, color: TEXT_HEADING, margin: "0 0 8px" }}>{s.title}</h3>
                <p style={{ fontSize: 12.5, color: TEXT_MUTED, lineHeight: 1.55, margin: 0 }}>{s.body}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className={styles.sectionPad} style={{ background: CREAM }}>
          <div className={styles.aboutGrid} style={{ display: "grid", alignItems: "center" }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: TEXT_MUTED, lineHeight: 1.6, margin: "0 0 12px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {ABOUT_CARDS.map((c) => (
                <div key={c.title} style={{ display: "flex", alignItems: "center", gap: 12, background: "white", border: `1px solid ${BORDER}`, borderRadius: RADIUS.lg, padding: "14px 16px" }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: TEXT_HEADING }}>{c.title}</div>
        ...
        <section className={styles.sectionPad}>
          ...
          <p style={{ fontSize: 13, color: TEXT_MUTED, lineHeight: 1.55, maxWidth: 560, margin: "0 0 20px" }}>
          ...
          <div className={styles.guidesGrid} style={{ display: "grid" }}>
            {GUIDE_COLUMNS.map((col) => (
              <div key={col.heading} style={{ background: "white", border: `1px solid ${BORDER}`, borderRadius: RADIUS.lg, padding: 20 }}>
                <h3 style={{ fontSize: 14, fontWeight: 800, color: TEXT_HEADING, margin: "0 0 12px" }}>{col.heading}</h3>
        ...
        <section className={styles.sectionPad} style={{ background: CREAM }}>
          <div style={{ maxWidth: 560, margin: "0 auto" }}>
            ...
            <p style={{ fontSize: 13, color: TEXT_MUTED, lineHeight: 1.55, margin: "0 0 20px", textAlign: "center" }}>
        ...
        <section className={styles.sectionPad} style={{ background: CREAM }}>
          ...
          <div className={styles.testimonialsGrid} style={{ display: "grid" }}>
            {TESTIMONIALS.map((t) => (
              <div key={t.author} style={{ background: "white", border: `1px solid ${BORDER}`, borderRadius: RADIUS.lg, padding: 16 }}>
                <div style={{ display: "flex", gap: 3, marginBottom: 10 }}>
                  {Array.from({ length: 5 }).map((_, i) => (
                    <StarIcon key={i} size={12} color={GOLD} />
                  ))}
                </div>
                <p style={{ fontSize: 12.5, color: TEXT_MUTED, lineHeight: 1.5, margin: "0 0 12px" }}>&quot;{t.quote}&quot;</p>
        ...
        <section className={styles.sectionPad}>
          ...
          <div className={styles.locationsGrid} style={{ display: "grid" }}>
            {LOCATIONS.map((loc) => (
              <div key={loc.slug}>
                <div style={{ fontSize: 13, fontWeight: 800, color: TEXT_HEADING, marginBottom: 6 }}>{loc.city}</div>
        ...
        <section className={styles.sectionPad} style={{ textAlign: "center", background: GRADIENT_TEAL }}>
          ...
          <p style={{ fontSize: 13, color: "oklch(0.9 0.03 292)", margin: "0 0 20px" }}>Free to compare, no obligation, and no sales calls afterwards.</p>
          <Link
            href="/get-a-quote"

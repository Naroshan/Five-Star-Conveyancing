...
export type UpdateDisbursementRuleInput = Partial<
  Pick<
    CreateDisbursementRuleInput,
    | 'chargeName'
    | 'category'
    | 'amountType'
    | 'amount'
...
async function fetchDisbursementOrThrow(db: Kysely<Database>, disbursementId: string): Promise<DisbursementRule> {
  const row = await db.selectFrom('disbursement_rules').selectAll().where('disbursement_id', '=', disbursementId).executeTakeFirst();
  if (!row) throw new InvalidStateError(`Disbursement rule ${disbursementId} does not exist.`);
  return mapDisbursementRule(row);
}

export async function createDisbursementRuleDraft(
  db: Kysely<Database>,
  user: AdminUser,
...
export async function updateDisbursementRuleDraft(
  db: Kysely<Database>,
  user: AdminUser,
  disbursementId: string,
  updates: UpdateDisbursementRuleInput
): Promise<DisbursementRule> {
...
export async function submitDisbursementRuleForReview(db: Kysely<Database>, user: AdminUser, disbursementId: string): Promise<DisbursementRule> {
...
export async function approveDisbursementRule(db: Kysely<Database>, user: AdminUser, disbursementId: string): Promise<DisbursementRule> {
...
export async function rejectDisbursementRule(db: Kysely<Database>, user: AdminUser, disbursementId: string, reason: string): Promise<DisbursementRule> {
  assertPermission(user, 'disbursements:approve');

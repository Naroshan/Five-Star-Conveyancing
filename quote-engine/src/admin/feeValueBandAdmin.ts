...
export type UpdateFeeValueBandInput = Partial<
  Pick<CreateFeeValueBandInput, 'valueMin' | 'valueMax' | 'boundaryRule' | 'baseFee' | 'effectiveDate' | 'expiryDate'>
>;

async function fetchBandOrThrow(db: Kysely<Database>, bandId: string): Promise<FeeValueBand> {
  const row = await db.selectFrom('fee_value_bands').selectAll().where('band_id', '=', bandId).executeTakeFirst();
  if (!row) throw new InvalidStateError(`Fee value band ${bandId} does not exist.`);
...
export async function updateFeeValueBandDraft(
  db: Kysely<Database>,
  user: AdminUser,
  bandId: string,
  updates: UpdateFeeValueBandInput
): Promise<FeeValueBand> {
...
export async function submitFeeValueBandForReview(db: Kysely<Database>, user: AdminUser, bandId: string): Promise<FeeValueBand> {
...
export async function approveFeeValueBand(db: Kysely<Database>, user: AdminUser, bandId: string): Promise<FeeValueBand> {
...
export async function rejectFeeValueBand(db: Kysely<Database>, user: AdminUser, bandId: string, reason: string): Promise<FeeValueBand> {
  assertPermission(user, 'fee_bands:approve');

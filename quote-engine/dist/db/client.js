...
import pg from 'pg';
pg.types.setTypeParser(1700, (value) => parseFloat(value)); // numeric -> number
export function createDb(connectionString) {
    if (!connectionString) {
        throw new Error('A database connection string is required. Set DATABASE_URL — never hard-code credentials in source.');
    }
    return new Kysely({
